...
- **Listen mode — default**: when the user says "listen to minecraft" / "wait for my in-game requests" or similar, run `powershell -ExecutionPolicy Bypass -File listen.ps1` as a **background process** (Bash `run_in_background`) and tell the user you're listening. The script blocks until the player types `/claude <request>` in-game, then exits printing `PROMPT: <request>`. When notified of its completion: read the prompt from the task output, fulfill it YOURSELF with the `minecraft` MCP tools (send_chat ack → do → verify → send_chat result), then **immediately restart listen.ps1 in the background** and continue. Repeat forever until the user says stop.
- **Alternative — fully detached serving**: `serve.ps1` spawns a separate headless Claude (`claude -p`) that serves on its own (logs to `serve.log`). Use only when the user asks for a standalone/background server process.
